...
class CNN(nn.Module):
    def __init__(self, input_length, pool_size, filters, activation, DenseNum):
        ...
        additional_layers = [nn.Linear(32, 16)] + [nn.Linear(16, 16) for _ in range(DenseNum-3)]
...
class CNNModel(RegressionModel):
    def __init__(self, name: str = "MLP", task_name: str = "", seed: int = 2024, model_dir = None, results_dir = None, n_dim=None):
        super().__init__(name)
        self.X_val = None
        self.y_val = None 
        self.model = None
    ...
    def predict(self, X_test: np.ndarray) -> np.ndarray:
        X_test = torch.from_numpy(self.preprocessing.transform(X_test)).to(device=self.device, dtype=torch.float32)
        with torch.no_grad():
    ...
    def _data_preprocess(self, X_train, y_train, X_val, y_val):
        ...
        data = {
            part: {k: torch.as_tensor(v, device=self.device, dtype=torch.float32) for k, v in data_numpy[part].items()}
            for part in data_numpy
        }
    ...
    def optimize_hyperparameters(self, 
        ...
        def object_func(trial):
            ...
            for train_index, val_index in kf.split(X_train):
                ...
                # 训练模型
                self.fit(X_train_fold, y_train_fold, optimize_hyperparams=False, optimizing=True, save_model=False)
