...
class FTTransformerModel(RegressionModel):
    ...
    def fit(self, 
        ...
        self.optimizer = torch.optim.AdamW(self.model.make_parameter_groups(), lr=1e-4, weight_decay=1e-5)

        self.apply_fn = lambda batch, model: model(batch["x_cont"], batch.get("x_cat")).squeeze(-1)
